...
public class Part2 {
	...
	public static List<Unit> copyElfList = new ArrayList<>();
	public static List<Unit> gobList = new ArrayList<>();
	public static List<Unit> copyGobList = new ArrayList<>();
	...
	public static void main (String args[]) {
		...
		boolean elfsWin = false;
		int attPowerTested = 3;
		int roundCounter = 0;
		while (!elfsWin) {
			unitList = new ArrayList<>();
			elfList = new ArrayList<>();
			gobList = new ArrayList<>();
			for (Unit elf : copyElfList) {
				Unit newElf = elf.clone();
				newElf.position.occupied = true;
				newElf.attPower = attPowerTested;
				elfList.add(newElf);
			}
			for (Unit goblin : copyGobList) {
				Unit newGoblin = goblin.clone();
				newGoblin.position.occupied = true;
				gobList.add(newGoblin);
			}
			...
			if (elfList.size() == copyElfList.size()) {
				elfsWin = true;
			} else {
				attPowerTested++;
			}
...
class Unit implements Comparable<Unit> {
	...
	public Unit clone() {
		return new Unit(this.isElf, this.position);
	}
