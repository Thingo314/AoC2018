...
public class Part2 {
	...
	static ArrayList<Unit> immuneCopy = new ArrayList<>();
	static ArrayList<Unit> infectorCopy = new ArrayList<>();
	...
	public static void main(String[] args) {
		...
		while (sc.hasNextLine()) {
			...
			if (takingImmuneUnits) {
				immuneCopy.add(new Unit(unitNum, hp, weaknesses, immunities, attDmg, attType, init));
			} else {
				infectorCopy.add(new Unit(unitNum, hp, weaknesses, immunities, attDmg, attType, init));
			}
		}

		sc.close();

		boolean immuneSysWon = false;
		int boost = 1;
		while (!immuneSysWon) {
			immuneUnits = new ArrayList<>();
			infectorUnits = new ArrayList<>();

			for (Unit unit : immuneCopy) {
				Unit copy = unit.clone();
				copy.addBoost(boost);
				immuneUnits.add(copy);
			}
			for (Unit unit : infectorCopy) {
				Unit copy = unit.clone();
				infectorUnits.add(copy);
			}
			...
			while (immuneUnits.size() != 0 && infectorUnits.size() != 0) {
				...
				for (int i = 0; i < armies.size(); i++) {
					...
					if (attackingUnit.getUnits() > 0 && attackTargets.containsKey(attackingUnit)) {
						...
						int unitsKilled = attackedUnit.loseUnits(attackedUnit.getPotentialDmg(attackingUnit));
						if (unitsKilled > 0)
							attacked = true;
					}
				}
			...
			if (attacked && immuneUnits.size() != 0) {
				immuneSysWon = true;
			} else {
				boost++;
			}
